...
5. **막차 임박 Web Push 알림** — 실제 브라우저 Push 알림(탭이 닫혀 있어도 수신). "탑승 가능 시간" 기준 **3시간/2시간 59분/2시간 58분 전 고정 3회** 발송되며, 각 알림에는 팀이 제작한 캐릭터 배너 이미지가 함께 표시된다(현재는 안드로이드만 지원).
...
- **2026-09-12 아키텍처 개편(진행 중): 경로탐색 제공자를 ODsay → 카카오맵 대중교통 경로 조회 API로 전환.** ODsay Basic 플랜의 일 30건 한도가 실사용(경로 후보당 구간별 막차 조회가 따라붙어 검색 1회에 최대 13건 소모)으로 금방 소진되는 문제 때문. 카카오맵 API(`dapi.kakao.com/v2/routing/publictraffic`, 기존 카카오 REST 키로 심사 없이 즉시 사용 가능, 무료 할당량 월 30만 건 수준)로 경로탐색·지도표시를 옮기되, **카카오 응답에는 역·정류장 ID가 전혀 없고 이름만 있어** 막차시각 조회(ODsay `searchSubwaySchedule`/TOPIS `getBustimeByStation`)에 필요한 ID로 변환하는 중간 계층이 새로 필요함. 상세 설계는 4장·10.1 참고. 롤백을 위해 개편 이전 상태를 git 태그 `odsay-stable`로 백업해둠(`git checkout odsay-stable`로 완전 복구 가능).
  - 지하철: ODsay `searchStation`(역명 검색 → stationID) API로 역ID를 얻어 기존 막차 조회 로직 재사용. **역명·노선명 → stationID 매핑은 영구 불변이라 무기한 캐시**하여 반복 조회 비용을 없앰. 방향(상행/하행) 확정에 쓰던 `wayCode`는 ODsay 경로탐색 응답에서만 나오는 값이라 카카오 전환 후에는 얻을 수 없어, 1차 구현에서는 예전의 "상/하행 중 이른 시각 채택" 근사로 되돌아감(추후 서울교통공사 API 연동 시 개선 예정, 아래 항목 참고)
  - **후속 최적화(서울교통공사 `data.seoul.go.kr` 자체 인증키 발급 필요, 아직 미보유):** 1~9호선 구간은 서울교통공사 무료 API(역명 검색 → 역코드 → 노선별 역 순서로 상/하행 판별 → 첫차·막차 조회)로 우선 처리해 ODsay 호출을 사실상 0에 가깝게 만들고, 신분당선·경의중앙선 등 그 외 노선만 ODsay로 폴백. 이렇게 하면 방향 확정도 카카오가 주는 정류장 순서(이동 순서대로 나열됨)와 노선의 정규 역 순서를 비교해 정확히 판별할 수 있어 `wayCode` 없이도 지하철 막차 버그를 재발시키지 않음
  - 버스: 카카오가 주는 정류장 이름 + 노선명으로 TOPIS `getStationByPos`(좌표 반경 검색) 결과 중 이름이 일치하는 후보를 찾아 arsId 확정. 동명이정류장(방향이 다른 같은 이름의 정류장)이 여러 개 나오면, 그 노선의 정류장 순서(TOPIS `노선정보조회서비스` — 별도 활용신청 필요, 자동승인)를 조회해 "다음 정류장 이름이 일치하는 쪽"으로 정확히 방향을 확정. 1차 구현에서는 이 순서비교 없이 후보가 1개로 유일할 때만 처리하고, 모호하면 안전하게 mock으로 폴백(순서비교는 키 발급 후 추가 예정)
...
### F5. 막차 임박 Web Push 알림

- **목적**: 사용자가 앱을 보고 있지 않아도 막차 준비 시점을 놓치지 않도록, "탑승 가능 시간"이 다가온다는 사실을 미리 알린다.
- **입력**: F3에서 계산된 "탑승 가능 시간"(departureDeadline)
- **처리 로직**
  - 프론트엔드에서 Push 구독을 생성해 백엔드에 저장
  - 백엔드가 departureDeadline 기준 **정확히 3시간 / 2시간 59분 / 2시간 58분 전** 3개 시점마다 각 1회씩 고정 문구("3시간 남았어요!" / "2시간 59분 남았어요!" / "2시간 58분 남았어요!")로 Web Push 발송 (VAPID 기반, web-push 라이브러리 사용, 10초 간격으로 순회하며 분 단위 경계를 감지)
  - 각 알림에는 팀이 제작한 캐릭터 배너 이미지(1200×600, 안드로이드 "큰 이미지" 알림용)를 함께 실어 보낸다 — iOS Safari는 이 이미지 옵션 자체를 지원하지 않아 자동으로 텍스트만 표시됨(현재는 안드로이드 대상으로만 적용)
- **출력/화면**: OS 알림 센터에 표시되는 Push 알림, 알림 클릭 시 앱으로 복귀
- **의존성/제약**: iOS는 16.4 이상 + 홈 화면 추가 PWA 상태에서만 수신 가능 (F1 온보딩에서 "홈 화면에 추가" 유도를 바로 노출)
...
>
> **v2.4 변경**: "위험도 등급(안전/주의/위험)이 바뀔 때마다 알림을 갱신"하던 기존 방식을 완전히 제거하고, **departureDeadline 기준 3시간/2시간 59분/2시간 58분 전 고정 3회 알림**으로 교체. 캐릭터 표정을 등급마다 동적으로 갱신하는 대신, 세 알림에 각각 안전/주의/위험 무드의 배너 이미지를 고정으로 매칭하는 방식으로 단순화. F4 화면 안의 환승구간별 안전/주의/위험 색상 배지(캐릭터 상태 표시 포함)는 이 변경과 무관하게 그대로 유지된다 — 이번 변경은 Push 발송 스케줄에만 해당.
...
4. 앱을 보고 있지 않아도 "탑승 가능 시간" 3시간/2시간 59분/2시간 58분 전에 F5 Push 알림으로 안내받는다.
...
- **경로탐색 제공자 추상화(2026-09-12 추가)**: `ROUTING_PROVIDER` 환경변수(`odsay` | `kakao`)로 경로탐색 소스를 전환할 수 있게 구현한다. 기존 ODsay 연동 코드는 삭제하지 않고 `services/providers/odsay.js`로, 신규 카카오 연동은 `services/providers/kakao.js`로 분리해 언제든 환경변수만 되돌려 즉시 롤백 가능하게 한다(1.6 참고, 개편 이전 상태는 git 태그 `odsay-stable`로 백업됨).
...
PushSubscription {
  ...
  transferGaps: [...]
  notifiedCountdownStages: string[]  // 이미 보낸 stage key 목록(중복 발송 방지)
}
```
백엔드 스케줄러는 주기적으로(10초 간격) 저장된 트립들을 순회하며, `departureDeadline` 기준 정확히 180/179/178분(3시간/2시간 59분/2시간 58분) 남은 시점을 감지해 해당 stage가 `notifiedCountdownStages`에 없으면 그 시점에 맞는 문구 + 캐릭터 배너 이미지로 Push를 1회 발송하고 stage를 기록한다.

> **v2.4 변경**: 기존에는 위험도 등급(안전/주의/위험)이 바뀔 때마다 Push를 갱신하는 방식(`lastNotifiedSafety` 필드로 추적)이었으나, **departureDeadline 기준 3시간/2시간 59분/2시간 58분 전 고정 3회 알림**으로 완전히 교체하면서 `lastNotifiedSafety` 필드도 제거하고 `notifiedCountdownStages`로 대체.
...
- **카카오맵 "대중교통 경로 조회" API**(`GET https://dapi.kakao.com/v2/routing/publictraffic`, `start_x`/`start_y`/`end_x`/`end_y` 파라미터) — 2026-06-16 공지, 2026-07-21부터 이미 서비스 중인 신규 오픈API. **실제 호출로 검증 완료**: 기존 카카오 REST 키로 별도 심사·설정 없이 바로 동작하며, 한 번 호출에 경로 후보 15개(지하철 3·버스 8·버스+지하철 4)를 반환하고 구간별 전체 경유 정류장 목록·운임(fare)·지도용 좌표(polyline)까지 포함 — ODsay보다 후보 수·데이터 풍부함 모두 우수하고, 실사용 사례 기준 월 30만 건 수준의 무료 할당량으로 ODsay(일 30건)와는 비교가 안 됨. **다만 응답의 정류장/노선 정보에 이름(`name`)만 있고 역ID·정류장ID(arsId)·노선ID·방향 필드가 전혀 없어**, 막차시각 조회(TOPIS/ODsay 스케줄 API)에 필요한 ID로 바로 연결할 수 없다 — 이름만으로 역/정류장을 특정하려 하면 오늘 확인한 것과 같은 동명이역(같은 이름, 다른 방향/폴) 오매칭 위험이 그대로 재현된다. **경로탐색·지도 표시 레이어만 이 API로 교체하고, 막차 계산용 ID는 좌표 기반 근접 정류장 검색 등 별도 방식으로 보완하는 하이브리드 구조**를 향후 검토할 가치가 있음(단, 카카오 로컬 API 약관상 응답 캐싱·저장이 금지되어 있어 항상 실시간 호출해야 하는 제약 있음 — 카카오 담당자 데브톡 답변 기준).
...
- **[카카오모빌리티 "대중교통 통합 길찾기"(multimodal/publictransit) API](https://developers.kakaomobility.com/affiliate/publictransit/start.html)로 경로탐색 전환 — ODsay 일 30건 한도 문제의 근본적인 해결책으로 향후 검토.** 응답에 지하철/버스 구분(`transport_type`), 노선ID(`route_id`), 방향(`UP`/`DOWN`), 정류장/역 ID(`stop_id`), 첫차·막차 여부(`first_trip`/`last_trip`)가 모두 포함되어 있어 ODsay+TOPIS 이중 구조 없이 경로탐색 한 번으로 막차 판단까지 가능한, 지금까지 조사한 것 중 데이터 모델이 가장 우수한 옵션. B2B 제휴 상품이라 기업/서비스 단위 계약 구조로 운영되는 것으로 보여, 개인 개발자용 ODsay Basic(일 30건)보다 훨씬 넉넉한 호출 한도가 기대되나 **정확한 일일 한도·요금은 공개 문서에 없어 온보딩 과정에서 제휴팀에 직접 확인 필요**.
  - **온보딩 절차(공식 가이드 기준)**: ① 카카오비즈니스 계정 생성(기존 카카오디벨로퍼스 계정 연동 가능) → ② 마이인포에서 기본 정보(회사명 등) 설정 → ③ 카카오 이메일·앱 ID·앱 이름·회사명을 `tech.partners@kakaomobility.com`으로 제출 → ④ 계정 승인 완료 후 이용 가능. 승인 소요 기간은 문서에 명시되어 있지 않음(해커톤처럼 당일 승인이 필요한 일정에는 부적합, 정식 서비스 전환 시 미리 신청해둘 것)
  - 전환 시 `backend/src/services/odsay.js`(경로탐색)와 `lastTrain.js`의 지하철 막차 조회 로직을 이 API 응답 스키마에 맞게 새로 작성해야 하며, 방향 필드(`UP`/`DOWN`)는 ODsay의 `wayCode`와 동일한 역할을 하므로 오늘 고친 방향 확정 로직의 설계는 그대로 재사용 가능
