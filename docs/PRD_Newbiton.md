...
- **카카오모빌리티 "대중교통 통합 길찾기"(multimodal/publictransit) API 제휴 검토** — 응답에 지하철/버스 구분(`transport_type`), 노선ID(`route_id`), 방향(`UP`/`DOWN`), 정류장/역 ID(`stop_id`), 첫차·막차 여부(`first_trip`/`last_trip`)가 모두 포함되어 있어 ODsay+TOPIS 이중 구조 없이 경로탐색 한 번으로 막차 판단까지 가능한, 지금까지 조사한 것 중 데이터 모델이 가장 우수한 옵션. 다만 일반 개발자 셀프서비스가 아니라 **제휴팀 문의(`tech.partners@kakaomobility.com`, 회사명·제휴 목적·예상 사용량 필요) 후 검토/계약 절차**가 있어 당일 사용은 불가능 — 서비스를 정식으로 키울 경우 1순위로 검토할 것
